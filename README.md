...
- Someone in a group can start a new play session and invite players from that group via real-time invites.
- Invited players receive real-time invites and can join the play session.
- Once players are in the play session, the creator becomes the host. Only the host can start/end matches and assign win/loss.
- Host can start a match, and a random selection of non-spectator players is selected and teams are balanced based on their Elo scores. Ideally all players should see similar gametime within a session.
...
- Groups
  ...
  - Groups have no "admins", everyone in the group is equal and can invite others. Anyone within the group can create a play session.
  ...
  - Within the same group, there can be multiple play sessions running simultaneously.

- Play Sessions

  - Play sessions exist within groups. At the start of a day/night/event, someone in the group can start a new play session and invite players from that group.
  - Invited players receive real-time invites via SSE and can join the play session.
  - The creator of the play session becomes the host. Only the host can start/end matches and assign win/loss.
  - Players can leave a play session at any point if they desire.
  - Host can invite more players than there are game slots (before and during the play session).
  - Host can manually assign spectators within the play session.
...
- Match
  ...
  - Matches exist within play sessions.
  - Match size is configurable and there's no size limit
  - When a match is started, a random selection of non-spectator players from the play session is selected and teams are balanced based on their Elo scores. The algorithm should ideally ensure all players see similar gametime within a session.
  ...
  - A player cannot be in multiple active matches within the same play session.
...
- UI
  ...
  - When a host starts a match, the Elo app randomly selects non-spectator players from the play session, optimizes the game and picks the teams based on selected player's elos (using average Elo for balance, choosing the closest balance if perfect balance isn't possible, ideally ensuring similar gametime for all players) and the host will be able to click Start when the match officially begins.
